...
class CronSchedule
{
	...
	// ION
	final public function current($time = FALSE)
	{
		// Convert parameters to array datetime
		$arrDT = $this->dtFromParameters($time);
		...
		while ( 1 )
		{
			...
			$daysInPreviousMonth = date('t', strtotime('month', strtotime($arrDT[4] . '-' . $arrDT[3])));
			if ($this->recedeItem($this->_minutes, 0, 59, $arrDT[0]))
				if ($this->recedeItem($this->_hours, 0, 23, $arrDT[1]))
					if ($this->recedeItem($this->_daysOfMonth, 0, $daysInPreviousMonth, $arrDT[2]))
						if ($this->recedeItem($this->_months, 1, 12, $arrDT[3]))
	...
	final public function asNaturalLanguage()
	{
		...
		$txtMinutes[2] = $this->natlangRange($this->_cronMinutes, array(
				$this,
		...
		$txtHours[1] = array();
		$txtHours[1]['between'] = $this->natlangRange($this->_cronHours, array(
				$this,
				'natlangElementHour' 
		), TRUE);
		$txtHours[1]['past'] = $this->natlangRange($this->_cronHours, array(
				$this,
				'natlangElementHour' 
		), FALSE);
		$txtHours[2] = array();
		$txtHours[2]['between'] = $this->natlangRange($this->_cronHours, array(
				$this,
				'natlangElementHour' 
		), TRUE);
		$txtHours[2]['past'] = $this->natlangRange($this->_cronHours, array(
				$this,
				'natlangElementHour' 
		), FALSE);
		...
		$txtDaysOfMonth[2] = $this->natlangApply('elemDOM: on_X', $this->natlangRange($this->_cronDaysOfMonth, array(
				$this,
				'natlangElementDayOfMonth' 
		)));
		...
		$txtMonths[2] = $this->natlangApply('elemMonth: during_X', $this->natlangRange($this->_cronMonths, array(
				$this,
				'natlangElementMonth' 
		)));
		...
		if ($this->_cronYears)
		{
			...
			$txtYears[2] = ' ' . $this->natlangApply('elemYear: in_X', $this->natlangRange($this->_cronYears, array(
					$this,
					'natlangElementYear' 
			)));
